...
func CheckIsNeedSellOnTrendUp(currencyConfig *models.CurrencyNotifConfig, shortInterval, midInterval, longInterval models.BandResult, currentTime time.Time) bool {
	...
	if currencyConfig.HoldPrice > shortInterval.CurrentPrice {
		...
		if !isHasCrossUpper(longInterval.Bands[bandLen-4:], true) && !isHasCrossUpper(shortInterval.Bands[bandLen-4:], true) {
			if midInterval.Position == models.ABOVE_UPPER && countCrossUpperOnBody(midInterval.Bands[bandLen-4:]) == 1 && changesInPercent > 5 {
				reason = "mid cross upper and just one and percent more than 5"
				return true
			}
		}
		...
		if longInterval.AllTrend.ShortTrend == models.TREND_DOWN && isHasBandDownFromUpper(longInterval.Bands[bandLen-4:]) {
			if changesInPercent > 5 && (isHasOpenCloseAboveUpper(shortInterval.Bands[bandLen-2:]) || isHasUpperHeadMoreThanUpperBody(shortInterval.Bands[bandLen-2:])) {
				reason = "long contain band down from upper, short upper head more than body2"
				return true
			}
		}
