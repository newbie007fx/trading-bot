...
func IsIgnoredLongInterval(result *models.BandResult, shortInterval *models.BandResult, midInterval *models.BandResult) bool {
	...
	hMidFourthBand := midInterval.HeuristicBand.FourthBand
	...
	midHFourthPercentFromUpper := (hMidFourthBand.Upper - float64(hMidFourthBand.Candle.Close)) / float64(hMidFourthBand.Candle.Close) * 100
	...
	if result.Position == models.ABOVE_UPPER {
		...
		if midHigestIndex == bandLen-1 && midInterval.AllTrend.ShortTrend == models.TREND_UP && midInterval.Position == models.ABOVE_SMA {
			if !isHasCrossUpper(midInterval.Bands[bandLen-4:], true) && isHasCrossUpper(midInterval.Bands[bandLen-8:], true) {
				if shortHSecondPercentFromUpper < 3.2 {
					ignoredReason = "position above upper and higest but not cross upper"
					return true
				}
			}
		}
	...
	if midInterval.Position == models.ABOVE_SMA && shortInterval.Position == models.ABOVE_SMA {
		if result.Position == models.ABOVE_SMA {
			...
			if !isHasCrossUpper(result.Bands[bandLen-2:], true) && !isHasCrossUpper(midInterval.Bands[bandLen-2:], true) && !isHasCrossUpper(shortInterval.Bands[bandLen-2:], true) {
				if shortHSecondPercentFromUpper < 3.2 && midHFirstPercentFromUpper < 3.3 && percentFromUpper < 3.4 {
					ignoredReason = "all interval percent below 3"
					return true
				}
			}
		}
	...
	if !(isHasCrossLower(shortInterval.Bands[bandLen-7:], false) && midInterval.Position == models.ABOVE_SMA && result.Position == models.ABOVE_SMA && midInterval.Direction == BAND_UP && result.Direction == BAND_UP) {
		...
		if result.Position == models.ABOVE_SMA && result.AllTrend.Trend == models.TREND_UP && result.AllTrend.ShortTrend == models.TREND_DOWN {
			if midInterval.AllTrend.FirstTrend == models.TREND_UP && midInterval.AllTrend.SecondTrend == models.TREND_DOWN {
				if shortInterval.AllTrend.FirstTrend == models.TREND_DOWN && shortInterval.AllTrend.SecondTrend == models.TREND_DOWN && !isHasCrossLower(shortInterval.Bands[bandLen-2:], false) {
		...
		if result.AllTrend.ShortTrend == models.TREND_DOWN && higestHigest != len(result.Bands)-1 && higestHigest > len(result.Bands)-5 {
			...
			if percent > 10 && midInterval.AllTrend.SecondTrend == models.TREND_DOWN && !isHasCrossLower(midInterval.Bands[len(midInterval.Bands)/2:], false) {
				if !isHasCrossUpper(result.Bands[bandLen-2:], true) || !isHasCrossLower(shortInterval.Bands[bandLen-2:], false) {
	...
	if (result.AllTrend.Trend == models.TREND_DOWN || result.AllTrend.SecondTrend == models.TREND_DOWN) && result.Position == models.BELOW_SMA {
		...
		if midInterval.Position == models.BELOW_SMA {
		...
		if countBelowSMA(result.Bands[len(result.Bands)-7:], false) == 7 {
	...
	if result.Position == models.ABOVE_SMA {
		if isHasCrossUpper(result.Bands[:len(result.Bands)/2], true) {
			...
			if midInterval.AllTrend.ShortTrend != models.TREND_UP && countCrossSMA(midInterval.Bands[bandLen-4:]) > 2 {
				ignoredReason = "already cross upper and now down"
				return true
			}
		}
		...
		if isHasOpenCloseAboveUpper(result.Bands[len(result.Bands)-7:]) {
		...
		if result.Direction == BAND_DOWN {
			if midInterval.AllTrend.ShortTrend != models.TREND_UP && isHasCrossUpper(midInterval.Bands[bandLen-4:], false) {
				if shortInterval.Position == models.BELOW_SMA && shortHSecondPercentFromSMA < 3.1 {
					ignoredReason = "band down above sma. and mid interval already cross upper"
					return true
				}
			}
		}
	...
	if isHasOpenCloseAboveUpper(result.Bands[bandLen-4:]) {
		if midInterval.AllTrend.ShortTrend == models.TREND_DOWN || shortInterval.AllTrend.SecondTrend == models.TREND_DOWN {
			if countCrossSMA(shortInterval.Bands[bandLen-4:]) > 2 {
				ignoredReason = "contain open close above upper and start down"
				return true
			}
		}

		if isHasOpenCloseAboveUpper(midInterval.Bands[bandLen-4:]) {
			ignoredReason = "contain open close above upper and mid interval contain open close above upper too"
			return true
		}
	}
	...
	if result.AllTrend.SecondTrend == models.TREND_DOWN {
		...
		if hightIndex >= len(midInterval.Bands)/2-3 && midInterval.Bands[hightIndex].Candle.Hight > float32(midInterval.Bands[hightIndex].Upper) {
			if countBelowSMA(midInterval.Bands[hightIndex:], false) > 0 && countBelowSMA(midInterval.Bands[:len(midInterval.Bands)/2], false) > 0 {
				if !(isHasCrossLower(midInterval.Bands[bandLen-2:], false) && midInterval.AllTrend.ShortTrend == models.TREND_UP) {
					if midHFirstPercentFromUpper < 3.3 {
	...
	if result.AllTrend.SecondTrend == models.TREND_SIDEWAY && countAboveSMA(result.Bands[len(result.Bands)-7:]) == 0 {
		if result.Position == models.BELOW_SMA && hFirstPercentFromSMA < 3.3 {
			if (midInterval.Position == models.BELOW_SMA && midHFirstPercentFromSMA < 3.2) || (midInterval.Position == models.ABOVE_SMA && midHFirstPercentFromUpper < 3.3) || midInterval.Position == models.ABOVE_UPPER {
	...
	if result.AllTrend.Trend == models.TREND_DOWN && result.AllTrend.ShortTrend == models.TREND_DOWN {
		if !(isHasCrossLower(result.Bands[bandLen-1:], false) && (midInterval.AllTrend.ShortTrend == models.TREND_UP || isHasCrossLower(midInterval.Bands[bandLen-2:], false)) && (shortInterval.AllTrend.ShortTrend == models.TREND_UP || isHasCrossLower(shortInterval.Bands[bandLen-2:], false))) {
			if !(isHasCrossLower(midInterval.Bands[bandLen-2:], false) && midInterval.AllTrend.ShortTrend == models.TREND_UP) {
				if isHasCrossSMA(shortInterval.Bands[len(shortInterval.Bands)-2:], false) || isHasCrossUpper(shortInterval.Bands[len(shortInterval.Bands)-2:], true) {
					if shortInterval.Position == models.ABOVE_SMA && shortHSecondPercentFromUpper < 3.2 {
			...
			if midInterval.AllTrend.Trend == models.TREND_DOWN && midInterval.AllTrend.ShortTrend == models.TREND_UP && !isHasCrossLower(midInterval.Bands[bandLen-2:], false) {
	...
	if result.Position == models.ABOVE_SMA && result.AllTrend.FirstTrend == models.TREND_UP && result.AllTrend.SecondTrend == models.TREND_UP {
		...
		if result.AllTrend.ShortTrend == models.TREND_DOWN {
			if midInterval.AllTrend.ShortTrend == models.TREND_DOWN && midHFourthPercentFromUpper < 3.3 {
				if shortInterval.AllTrend.SecondTrend == models.TREND_DOWN && shortHFourthPercentFromUpper < 3.2 {
					ignoredReason = "up trend, start down and short percent below 3"
					return true
				}
			}
		}
	...
	if result.AllTrend.SecondTrend == models.TREND_DOWN && result.AllTrend.ShortTrend != models.TREND_UP {
		...
		if midInterval.Position == models.BELOW_SMA && !isHasCrossLower(midInterval.Bands[bandLen-4:], false) {
			if countBelowLower(shortInterval.Bands[bandLen/2:], false) > 0 {
				if (shortInterval.Position == models.BELOW_SMA && shortHSecondPercentFromSMA < 3.1) || (shortInterval.Position == models.ABOVE_SMA && shortHSecondPercentFromUpper < 3.2) {
					ignoredReason = "down trend, short contain open close below lower, short cross percent below 3"
					return true
				}
	...
	if result.AllTrend.ShortTrend == models.TREND_UP && result.Direction == BAND_DOWN {
		if midInterval.AllTrend.FirstTrend == models.TREND_UP && midInterval.AllTrend.SecondTrend == models.TREND_UP && midInterval.AllTrend.ShortTrend == models.TREND_DOWN {
			if shortInterval.AllTrend.ShortTrend != models.TREND_UP || !(isHasCrossSMA(midInterval.Bands[bandLen-2:], false) && midInterval.Direction == models.TREND_UP) {
				if shortInterval.Position == models.BELOW_SMA && shortHSecondPercentFromSMA < 3.1 {
	...
	if result.AllTrend.ShortTrend == models.TREND_DOWN {
		if midInterval.AllTrend.ShortTrend == models.TREND_DOWN && !(isHasCrossLower(shortInterval.Bands[bandLen-7:], false) && midInterval.Position == models.ABOVE_SMA && result.Position == models.ABOVE_SMA && midInterval.Direction == BAND_UP && result.Direction == BAND_UP) {
	...
	if result.AllTrend.SecondTrend == models.TREND_DOWN && result.Position == models.BELOW_SMA && isHasCrossLower(result.Bands[len(result.Bands)-2:], false) {
		if !((midInterval.AllTrend.ShortTrend == models.TREND_UP || isHasCrossLower(midInterval.Bands[bandLen-2:], false)) && (shortInterval.AllTrend.ShortTrend == models.TREND_UP || isHasCrossLower(shortInterval.Bands[bandLen-2:], false))) {
			...
			if !(isHasCrossLower(midInterval.Bands[bandLen-2:], false) && midInterval.AllTrend.ShortTrend == models.TREND_UP) {
				...
				if countDownBand(midInterval.Bands[len(midInterval.Bands)-4:len(midInterval.Bands)-1]) == 3 {
					...
					if shortHSecondPercentFromSMA < 3.2 {
						ignoredReason = "trend down, short percent from sma below 3 2nd"
						return true
					}
				}
			}
	...
	if result.AllTrend.ShortTrend == models.TREND_DOWN {
		if !(isHasCrossLower(result.Bands[bandLen-1:], false) && (midInterval.AllTrend.ShortTrend == models.TREND_UP || isHasCrossLower(midInterval.Bands[bandLen-2:], false)) && (shortInterval.AllTrend.ShortTrend == models.TREND_UP || isHasCrossLower(shortInterval.Bands[bandLen-2:], false))) {
			if midInterval.AllTrend.SecondTrend == models.TREND_DOWN && midInterval.AllTrend.ShortTrend == models.TREND_UP && !isHasCrossLower(midInterval.Bands[bandLen-2:], false) {
	...
	if result.AllTrend.ShortTrend != models.TREND_DOWN && (result.AllTrend.SecondTrend == models.TREND_DOWN || result.AllTrend.Trend == models.TREND_DOWN) {
		if midInterval.AllTrend.ShortTrend == models.TREND_UP {
	...
	if result.AllTrend.SecondTrend == models.TREND_UP && result.AllTrend.ShortTrend != models.TREND_UP {
		...
		if (midInterval.Position == models.BELOW_SMA && midHFirstPercentFromSMA < 3.2) || (midInterval.Position == models.ABOVE_SMA && midHFirstPercentFromUpper < 3.3) {
			if shortHSecondPercentFromUpper < 3.2 || isHasCrossSMA(shortInterval.Bands[bandLen-2:], false) {
	...
	if result.AllTrend.SecondTrend == models.TREND_UP && result.AllTrend.ShortTrend == models.TREND_UP {
		if result.Position == models.ABOVE_SMA {
			...
			if midInterval.AllTrend.ShortTrend == models.TREND_DOWN && midInterval.Position == models.BELOW_SMA {
				if shortInterval.AllTrend.SecondTrend == models.TREND_DOWN && result.Position == models.ABOVE_SMA {
					ignoredReason = "long trend up but starting down, short just minor up"
					return true
				}
			}
		}
		...
		if result.Direction == BAND_DOWN {
			...
			if midInterval.AllTrend.ShortTrend == models.TREND_DOWN && isHasCrossUpper(midInterval.Bands[bandLen-4:], true) {
				if shortInterval.AllTrend.SecondTrend == models.TREND_DOWN && shortInterval.Position == models.BELOW_SMA && !isHasCrossLower(shortInterval.Bands[bandLen-4:], false) {
					ignoredReason = "band down, mid short trend down but percent from upper below 3 2nd"
					return true
				}
			}
		}
	...
	if result.AllTrend.Trend == models.TREND_DOWN && result.Position == models.BELOW_SMA && isHasCrossUpper(result.Bands[bandLen/2:], true) && !isHasCrossLower(result.Bands[bandLen-2:], false) {
		if midInterval.AllTrend.FirstTrend == models.TREND_DOWN && midInterval.AllTrend.ShortTrend == models.TREND_UP {
			if (midHFirstPercentFromSMA < 3.2 || hFirstPercentFromSMA < 3.4) && shortHSecondPercentFromUpper < 3.2 {
	...
	if result.AllTrend.SecondTrend == models.TREND_DOWN && !isHasCrossLower(result.Bands[bandLen-2:], false) {
		if midInterval.AllTrend.SecondTrend == models.TREND_UP && midInterval.AllTrend.ShortTrend == models.TREND_DOWN {
		...
		if isHasCrossLower(result.Bands[bandLen-4:], false) && hFirstPercentFromSMA < 3.4 {
			if midInterval.Position == models.BELOW_SMA && (midHFourthPercentFromUpper < 3.3) {
				if shortHFourthPercentFromUpper < 3.2 {
					ignoredReason = "down from uppper and short up percent from upper below 3 4nd"
					return true
				}
			}
		}
	...
	if isBandHeadDoubleBody(result.Bands[bandLen-2:]) || ((result.AllTrend.Trend == models.TREND_DOWN || result.AllTrend.SecondTrend == models.TREND_DOWN) && result.AllTrend.ShortTrend == models.TREND_DOWN) {
		...
		if shortInterval.Position != models.BELOW_SMA || shortInterval.AllTrend.ShortTrend != models.TREND_UP || !(isHasCrossSMA(midInterval.Bands[bandLen-2:], false) && midInterval.Direction == models.TREND_UP) {
	...
	if result.AllTrend.SecondTrend == models.TREND_UP && isHasCrossUpper(result.Bands[bandLen-2:], true) {
		...
		if midInterval.Position == models.ABOVE_SMA && (result.AllTrend.ShortTrend != models.TREND_UP || midInterval.AllTrend.ShortTrend != models.TREND_UP || shortInterval.AllTrend.ShortTrend != models.TREND_UP) {
			if result.Direction == BAND_DOWN || midInterval.Direction == BAND_DOWN || shortInterval.Direction == BAND_DOWN {
				if shortInterval.AllTrend.SecondTrend == models.TREND_UP && shortHSecondPercentFromUpper < 3.2 {
					ignoredReason = "cross upper and percent below 3 2nd"
					return true
				}
			}
		}
		...
		midHigestIndex := getHighestIndex(midInterval.Bands)
		midSecondHigestIndex := getHigestIndexSecond(midInterval.Bands)
		if hFirstPercentFromUpper < 3.4 {
			if midHigestIndex == bandLen-1 && midSecondHigestIndex < midHigestIndex-4 {
				if shortHSecondPercentFromUpper < 3.2 {
					ignoredReason = "cross upper and percent below 3 3nd"
					return true
				}
			}
		}
		...
		if midInterval.Direction == BAND_DOWN {
			ignoredReason = "cross upper and mid band down"
			return true
		}
	}
	...
	if result.AllTrend.SecondTrend == models.TREND_UP && result.Direction == BAND_DOWN {
		...
		if countAboveSMA(midInterval.Bands[bandLen/2:]) == 0 && midInterval.Position == models.BELOW_SMA && midHFirstPercentFromSMA < 3.2 {
			ignoredReason = "tren up but band down 2nd"
			return true
		}
	}
	...
	if result.AllTrend.ShortTrend == models.TREND_UP && (result.Direction == BAND_DOWN || percentFromUpper < 3.4 || isHasCrossUpper(result.Bands[bandLen-2:], true)) {
		if shortInterval.Position != models.BELOW_SMA || shortInterval.AllTrend.ShortTrend != models.TREND_UP || !(isHasCrossSMA(midInterval.Bands[bandLen-2:], false) && midInterval.Direction == models.TREND_UP) {
			if midInterval.AllTrend.SecondTrend == models.TREND_UP && CountUpBand(result.Bands[bandLen-3:]) != 3 {
	...
	if result.AllTrend.SecondTrend == models.TREND_DOWN && result.AllTrend.ShortTrend == models.TREND_DOWN {
		...
		if (midInterval.AllTrend.Trend == models.TREND_DOWN || midInterval.AllTrend.SecondTrend == models.TREND_DOWN) && midInterval.Position == models.BELOW_SMA {
			if (countBelowLower(shortInterval.Bands[bandLen-15:], false) > 0 || isHasCrossLower(shortInterval.Bands[bandLen/2:], true)) && shortHSecondPercentFromSMA < 3.1 {
				ignoredReason = "on down and just minor up 3nd"
				return true
			}
		}
		...
		if !(isHasCrossLower(result.Bands[bandLen-1:], false) && (midInterval.AllTrend.ShortTrend == models.TREND_UP || isHasCrossLower(midInterval.Bands[bandLen-2:], false)) && (shortInterval.AllTrend.ShortTrend == models.TREND_UP || isHasCrossLower(shortInterval.Bands[bandLen-2:], false))) {
			...
			if midInterval.AllTrend.SecondTrend == models.TREND_DOWN && midInterval.Position == models.BELOW_SMA && !(isHasCrossLower(midInterval.Bands[bandLen-2:], false) && midInterval.AllTrend.ShortTrend == models.TREND_UP) {
	...
	if result.Direction == BAND_DOWN || result.AllTrend.ShortTrend != models.TREND_UP {
		...
		if shortInterval.AllTrend.Trend == models.TREND_DOWN || shortInterval.AllTrend.SecondTrend == models.TREND_DOWN || (result.AllTrend.FirstTrend == models.TREND_DOWN && result.AllTrend.SecondTrend == models.TREND_SIDEWAY) || midInterval.Direction == BAND_DOWN {
			if (!isHasCrossLower(midInterval.Bands[bandLen-4:], false) && !isHasCrossSMA(midInterval.Bands[bandLen-4:], false)) || !isHasCrossLower(shortInterval.Bands[bandLen/2:], false) {
				if (!isHasCrossLower(shortInterval.Bands, false) || shortHSecondPercentFromSMA < 3.1) || shortHSecondPercentFromUpper < 3.2 {
					ignoredReason = "after cross upper band down, and short percent below 3"
					return true
				}
			}
		}
