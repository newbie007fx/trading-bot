...
func IsIgnored(result, masterCoin *models.BandResult, requestTime time.Time) bool {
	...
	if lastBand.Candle.Open >= float32(lastBand.Upper) || (secondLastBand.Candle.Open >= float32(secondLastBand.Upper) && secondLastBand.Candle.Close >= float32(secondLastBand.Upper)) {
...
func IsIgnoredMidInterval(result *models.BandResult, shortInterval *models.BandResult) bool {
	...
	if !upFromLower(result.Bands) && lastBand.Candle.Low > float32(lastBand.Lower) {
		ignoredReason = "not up from lower"
		return true
	}
	...
	if result.Position == models.BELOW_SMA && countCrossLower(result.Bands[len(result.Bands)-5:]) > 3 {
		if shortInterval.Position == models.ABOVE_SMA && !isHasCrossUpper(shortInterval.Bands, true) {
			if shortPercentFromUpper < 3 && percentFromHigest(result.Bands) < 3 {
...
func IsIgnoredLongInterval(result *models.BandResult, shortInterval *models.BandResult, midInterval *models.BandResult, masterTrend, masterMidTrend int8) bool {
	...
	if midInterval.Position == models.BELOW_SMA && result.Position == models.BELOW_SMA && result.AllTrend.SecondTrend == models.TREND_DOWN {
		if countBelowSMA(midInterval.Bands, false) == len(midInterval.Bands) && countBelowSMA(result.Bands[len(result.Bands)-5:], false) > 2 {
			if !isHasCrossLower(result.Bands[len(result.Bands)-3:], false) && (midInterval.AllTrend.ShortTrend != models.TREND_UP || !isHasCrossLower(midInterval.Bands[len(midInterval.Bands)-3:], false)) {
	...
	if downFromUpper(*result) {
		if midInterval.Position == models.BELOW_SMA && isHasCrossLower(midInterval.Bands, false) && !isHasCrossUpper(midInterval.Bands, true) && midPercentFromSMA < 3 {
	...
	midSMA := midLastBand.Upper - midLastBand.SMA
	percentMidSMA := (float32(midSMA) - midLastBand.Candle.Close) / midLastBand.Candle.Close * 100
	if result.AllTrend.Trend == models.TREND_DOWN {
		if !isHasCrossUpper(midInterval.Bands, true) && !isHasCrossSMA(midInterval.Bands[len(midInterval.Bands)/2:], true) {
			if isHasCrossUpper(shortInterval.Bands[len(shortInterval.Bands)-3:], true) && percentMidSMA < 3 {
				ignoredReason = "trend down from upper2"
				return true
			}
		}
	}
	...
	if result.AllTrend.SecondTrend == models.TREND_DOWN && result.AllTrend.ShortTrend != models.TREND_UP {
		if countAboveSMA(midInterval.Bands) > 2 && countCrossLower(midInterval.Bands) > 2 && countCrossUpper(midInterval.Bands) == 0 {
			price := GetHigestHightPrice(shortInterval.Bands)
			percentFromHightPrice := (price - shortLastBand.Candle.Close) / shortLastBand.Candle.Close * 100
			if percentFromHightPrice < 3 {
				ignoredReason = "short percent from higesh hight < 3"
				return true
			}
		}
...
func IsIgnoredMasterDown(result, midInterval, longInterval, masterCoin *models.BandResult, checkingTime time.Time) bool {
	...
	longSecondLastBand := longInterval.Bands[len(longInterval.Bands)-2]
	midSecondLastBand := midInterval.Bands[len(midInterval.Bands)-2]
	if longLastBand.Candle.Open < float32(longLastBand.SMA) && longLastBand.Candle.Close < float32(longLastBand.SMA) && hasCrossUpper(longInterval.Bands[len(longInterval.Bands)/2:]) {
		if longSecondLastBand.Candle.Open > float32(longSecondLastBand.SMA) {
			if midSecondLastBand.Candle.Close < float32(midSecondLastBand.Lower) {
				ignoredReason = "trend down, long down from sma"
				return true
			}
		}
	}
	...
	percentFromUpper := (lastBand.Upper - float64(lastBand.Candle.Close)) / float64(lastBand.Candle.Close) * 100
	midPercentFromUpper := (midLastBand.Upper - float64(midLastBand.Candle.Close)) / float64(midLastBand.Candle.Close) * 100
	if midPercentFromUpper < 3 && percentFromUpper < 3 {
		ignoredReason = "percent from upper < 3"
		return true
	}

	if (lastBand.Candle.Open < float32(lastBand.SMA) && lastBand.Candle.Hight > float32(lastBand.SMA)) || (lastBand.Candle.Open < float32(lastBand.Upper) && lastBand.Candle.Hight > float32(lastBand.Upper)) {
		if result.PriceChanges > 3 {
			ignoredReason = "cross SMA/Upper and price changes already more than 3"
			return true
		}
	}

	if midSecondLastBand.Candle.Close < float32(midSecondLastBand.Lower) {
		ignoredReason = "mid previous band close below lower"
		return true
	}

	midPercentFromSMA := (midLastBand.SMA - float64(midLastBand.Candle.Close)) / float64(midLastBand.Candle.Close) * 100
	if downFromUpper(*longInterval) && longLastBand.Candle.Open > float32(longLastBand.SMA) && longLastBand.Candle.Close < float32(longLastBand.SMA) {
		if midLastBand.Candle.Open < float32(midLastBand.Lower) && midPercentFromSMA < 3 {
			ignoredReason = "long down cross sma and mid margin from sma below 3"
			return true
		}
	}

	percentFromSMA := (float32(lastBand.SMA) - lastBand.Candle.Close) / lastBand.Candle.Close * 100
	if midInterval.AllTrend.SecondTrend == models.TREND_DOWN && midLastBand.Candle.Low > float32(midLastBand.Lower) {
		if midSecondLastBand.Candle.Close < midSecondLastBand.Candle.Open && midSecondLastBand.Candle.Low < float32(midSecondLastBand.Lower) {
			if !isHasCrossSMA(result.Bands, false) && result.AllTrend.SecondTrend == models.TREND_DOWN && percentFromSMA < 3 {
